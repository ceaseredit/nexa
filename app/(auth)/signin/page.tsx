...
//                     loading || !routingNumber.trim() || !password.trim()
//                   }
//                   className="w-full py-4 mt-5 font-bold tracking-wide text-white transition-opacity rounded-xl disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
//                   style={{ backgroundColor: COLORS.primaryBlack }}
//                 >
//                   {loading ? (
//                     <>
...
//                   className="w-full py-4 mt-4 font-bold tracking-wide text-white transition-opacity rounded-xl disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
//                   style={{ backgroundColor: COLORS.primaryBlack }}
//                 >
//                   {otpLoading ? (
//                     <>
...
import { COLORS } from "@/constants/Theme";
import { useDispatch, useSelector } from "react-redux";
import { userLogin, confirmSession } from "@/store/slices/userAuthSlice";
import { EyeIcon, EyeOffIcon } from "lucide-react";
import Link from "next/link";
import { useRouter } from "next/navigation";
import React, { useState } from "react";
...
function Page() {
  ...
  const handleCredentialsSubmit = async () => {
    ...
    if (userLogin.fulfilled.match(result)) {
      setResolvedUser(result.payload);
      // Keep spinner alive briefly before transitioning
  ...
  const handleOtpSubmit = async () => {
    ...
    if (otp === String(resolvedUser.otp)) {
      await dispatch(confirmSession(resolvedUser)); // ✅ persist session only now
      router.push("/dashboard");
    } else {
      setOtpLoading(false);
      setOtpError("Incorrect verification code. Please try again.");
